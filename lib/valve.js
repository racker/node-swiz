...
function normalizeIP(addr) {
  ...
  if (ip6.is4()) {
    // The ipv6 library will _always_ indicate too many groups in its
    // error field here.  To determine if we have a genuine IPv4 address
    // or not, or something else all-together, we attempt to prefix the
    // address with "::".  I'm using the following logic table:
    // 
    // IF                               THEN
    // "::"+addr parses as a valid      Return addr as-is, since it's a
    // IPv6 address                     valid IPv4 address.
    //
    // "::"+addr chokes AND ip6err      We actually have an IPv6 address.
    // reports no error                 Return its canonical form.
    //
    // "::"+addr chokes AND ip6err      We have a malformed IPv6 address
    // reports too many groups          with an embedded dotted-quad.

    var canon  = ip6.canonicalForm();
    var chk    = new ipv6.Address("::"+addr);
    if(typeof(chk.canonicalForm()) != 'undefined') {
      return addr;
    } else {
      if(!ip6.error && chk.error) {
        return canon;
      } else {
